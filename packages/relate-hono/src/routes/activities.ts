...
export function activityRoutes(pluralToSlug: Record<string, string>) {
  ...
  app.post('/activities', async (c) => {
    ...
    return c.json(await crm.activities.track({
      ...body,
  ...
  app.get('/activities', async (c) => {
    ...
    return c.json(await crm.activities.list(undefined, { type, limit, offset, before }))
  })
  ...
  app.get('/activities/:plural/:id', async (c) => {
    ...
    const offset = c.req.query('offset') ? Number(c.req.query('offset')) : undefined
    const before = c.req.query('before') ? new Date(c.req.query('before')!) : undefined
    return c.json(await crm.activities.list({ object: slug, id: c.req.param('id') }, { type, limit, offset, before }))
  })
